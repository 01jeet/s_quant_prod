...
# Try to create the DB, but ignore the error if it already exists
temporal-sql-tool --plugin postgres12 --ep ${POSTGRES_SEEDS} -u ${POSTGRES_USER} -p ${DB_PORT:-5432} --db temporal create || echo "Database already exists, skipping creation"
